...
export class Tab1Page implements OnInit {
  ...
  ngOnInit() {
    this.ejerciciosService.ejercicios$.subscribe(lista => {
  ...
  } else if (this.timerVisible && (ejercicioIndex === this.ejercicioActivoIndex)) {
  ...
  isUltimoSet(ejercicioIndex: number, setIndex: number): boolean {
    ...
    const ultimoSetIndex = this.ejercicios[ultimoEjercicioIndex].sets.length - 1;

    return (ejercicioIndex === ultimoEjercicioIndex) && (setIndex === ultimoSetIndex);
  }
